...
func TransferEth(client ethclient.Client, privKey string, to string, amount int64) (string, error) {
	...
	// Assuming you have connected a client, the next step is to load your private key
	privateKey, err := crypto.HexToECDSA(privKey)
	if err != nil {
		return "", err
	}

	// Function requires public address of the account we are sending from -- which we can derive from the private key
	...
	// We assign the transaction using  the sender's private key
